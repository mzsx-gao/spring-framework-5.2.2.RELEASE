...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		//获取事务(DataSourceTransactionObject)
		...
		//如果是PROPAGATION_REQUIRED，PROPAGATION_REQUIRES_NEW，PROPAGATION_NESTED这三种类型将开启一个新的事务
			...
			try {
				...
				/**
				 * DefaultTransactionStatus 中的 newTransaction 属性直接决定了提交，回滚和恢复绑定操作
				 * 如果为 true 就代表当前事务允许单独提交和回滚，一般是第一次创建事务或者事务传播属性为 PROPAGATION_REQUIRES_NEW 的时候
				 * 如果为 false 则当前事务不能单独提交和回滚
				 */
				...
				// 其实是给外围业务代码使用，外围业务代码可以通过TransactionSynchronizationManager的get操作获取这些属性从而做其它处理
	...
	private TransactionStatus handleExistingTransaction(
		...
		boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
		//如果第二次进来，传播行为是PROPAGATION_REQUIRED就会走到这里，此时newTransaction为false;在提交代码那里会判断这个属性，只有
		//true才会提交，否则不提交(源码位置:this.processCommit# if(status.isNewTransaction() )
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
                ...
                //如果当前事务不是独立的新事务，那么只能标记回滚状态（设置连接对象rollbackOnly为true），等到事务链执行完毕后统一回滚
				else {
